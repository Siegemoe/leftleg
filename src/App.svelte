  ...
  onMount(() => {
    ...
    void (async () => {
      ...
      const unlisten = await listen<PiEventEnvelope>("pi-event", (e) => {
        const p = e.payload;
        handleEvent(p.event, { project: p.project, proc: p.proc }).catch((e) =>
          reportError("event", String(e)),
        );
      });
      if (disposed) {
        unlisten();
        ...
        {#if $updateAvailable}
          <div class="update-banner">
            ...
            {:else if $updateStatus === "ready"}
              <button class="update-btn" onclick={() => void applyUpdate()}
