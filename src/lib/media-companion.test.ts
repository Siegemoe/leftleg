...
describe("media companion output durability", () => {
  ...
  it("uses the actual raster format when the response omits media_type", async () => {
    ...
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue(response([{ b64_json: gif.toString("base64") }])),
    );
  ...
  it("rejects malformed image data before saving any part of a batch", async () => {
    vi.stubGlobal(
      ...
      vi
        .fn()
        .mockResolvedValue(response([{ b64_json: png.toString("base64") }, { b64_json: "%%%%" }])),
    );
  ...
  it("keeps both images when identical prompts finish in the same millisecond", async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn().mockResolvedValue({
        ok: true,
        text: async () =>
          JSON.stringify({ data: [{ b64_json: payload, media_type: "image/png" }] }),
      }),
    );
    const context = {
      cwd: root,
