...
<header class="titlebar" data-tauri-drag-region>
  ...
  <nav class="menubar">
    <div class="menu">
      <button class="menu-label" class:open={openMenu === "file"} onclick={() => toggleMenu("file")}
        >File</button
      >
      {#if openMenu === "file"}
        <div class="dropdown">
          <button onclick={() => run(() => newSession())}
            >New Session{#if bindings.newSession}<span class="hint-key">{bindings.newSession}</span
              >{/if}</button
          >
          <button onclick={() => run(() => openNewProject())}
            >New Project…{#if bindings.newProject}<span class="hint-key">{bindings.newProject}</span
              >{/if}</button
          >
    ...
    <div class="menu">
      <button class="menu-label" class:open={openMenu === "edit"} onclick={() => toggleMenu("edit")}
        >Edit</button
      >
      {#if openMenu === "edit"}
        <div class="dropdown">
    ...
    <div class="menu">
      <button class="menu-label" class:open={openMenu === "view"} onclick={() => toggleMenu("view")}
        >View</button
      >
      {#if openMenu === "view"}
        <div class="dropdown">
          <button onclick={() => run(() => sidebarOpen.update((v) => !v))}
            >Toggle Sidebar{#if bindings.toggleSidebar}<span class="hint-key"
          ...
          <button onclick={() => run(() => openPanelTab("diff"))}
            >Diff…{#if bindings.openDiff}<span class="hint-key">{bindings.openDiff}</span
              >{/if}</button
          >
          <button onclick={() => run(() => openPanelTab("files"))}
            >Files…{#if bindings.openFiles}<span class="hint-key">{bindings.openFiles}</span
              >{/if}</button
          >
          <div class="sep"></div>
          <button onclick={() => run(() => applyTheme("light"))}
            >{$theme === "light" ? "✓ " : ""}Light Theme</button
          >
          <button onclick={() => run(() => applyTheme("dark"))}
            >{$theme === "dark" ? "✓ " : ""}Dark Theme</button
          >
          <button onclick={() => run(() => applyTheme("system"))}
    ...
    <div class="menu">
      <button class="menu-label" class:open={openMenu === "help"} onclick={() => toggleMenu("help")}
        >Help</button
      >
      {#if openMenu === "help"}
        <div class="dropdown">
  ...
  <div class="win-controls">
    <button class="win-btn" title="Minimize" onclick={() => void win.minimize()}
      ><Minus size={14} /></button
    >
    <button class="win-btn" title="Maximize / restore" onclick={() => void win.toggleMaximize()}
      ><Square size={11} /></button
    >
    <button class="win-btn close" title="Close" onclick={() => void win.close()}
      ><X size={15} /></button
    >
  </div>
</header>
...
{#if $aboutOpen}
  <div
    class="overlay"
    onclick={(e) => {
      if (e.target === e.currentTarget) aboutOpen.set(false);
    }}
    role="presentation"
  >
    <div class="about" role="dialog" aria-modal="true">
      ...
      <p class="desc">
        A control surface for the <a
          href="https://github.com/earendil-works/pi-mono"
          target="_blank"
          rel="noreferrer">Pi coding agent</a
        >.
      </p>
