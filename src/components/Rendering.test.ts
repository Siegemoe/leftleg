...
it("shows offline recovery instead of a permanent startup message after Pi exits", () => {
  ...
  items.set([]); projectDir.set("/work"); disconnected.set(true);
  instances.push(mount(Chat, { target: document.body })); flushSync();
...
afterEach(async () => { for (const i of instances.splice(0)) await unmount(i); document.body.replaceChildren(); vi.unstubAllGlobals(); });
it("scrolls on store updates and respects scrolling away from the bottom", () => {
  ...
  items.set([]); streaming.set(false); projectDir.set("/work"); composerDraft.set(null);
  instances.push(mount(Chat, { target: document.body })); flushSync();
