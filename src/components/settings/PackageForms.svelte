<script lang="ts">
  ...
  // Number fields: garbage like "12e" blurs to NaN, which JSON-serializes as
  ...
  const planFields: FieldDef[] = [
    { path: "planModel", label: "planModel", type: "string", hint: "model used for plan drafting" },
    {
      ...
      label: "planThinking",
      type: "string",
      hint: "thinking level for plan drafting",
    },
  ...
  function roleNames(target: string): string[] {
    const roles = (files[target]?.data?.subagent as { roles?: Record<string, unknown> } | undefined)
      ?.roles;
  ...
  async function addRole(target: string) {
    ...
    if (!name) return;
    if (roleNames(target).includes(name)) {
      statusNote.set("That role already exists");
      return;
    }
  ...
  async function deleteRole(target: string, role: string) {
    try {
      const roles = (
        files[target]?.data?.subagent as { roles?: Record<string, unknown> } | undefined
      )?.roles;
      if (!roles) return;
      const next = { ...roles };
      delete next[role];
      ...
      await mgmtRequest("write", {
        target,
        mode: "namespace",
        patch: { subagent: { ...subagent, roles: next } },
        revision: revisionFor(target),
      });
      await load(target);
      flashSaved();
    } catch (e) {
  ...
  function permPatterns(
    target: string,
    tool: string,
  ): Array<{ pattern: string; decision: Decision }> {
    const perm = files[target]?.data?.permission as Record<string, unknown> | undefined;
    const t = perm?.[tool];
    if (t === "allow" || t === "ask" || t === "deny") return [{ pattern: "*", decision: t }];
    if (t && typeof t === "object") {
      return Object.entries(t as Record<string, unknown>).map(([pattern, d]) => ({
        pattern,
        decision: (["allow", "ask", "deny"].includes(String(d)) ? d : "ask") as Decision,
      }));
    }
    return [];
  }
  async function savePermissionTool(
    target: string,
    tool: string,
    patterns: Array<{ pattern: string; decision: Decision }>,
  ) {
    ...
    try {
      await mgmtRequest("write", {
        target,
        mode: "namespace",
        patch: { permission: { ...permission, [tool]: value } },
        revision: revisionFor(target),
      });
      await load(target);
      flashSaved();
  ...
  async function saveDistillField(path: string, value: unknown) {
    try {
      ...
      await mgmtRequest("write", {
        target: "distill-config",
        mode: "merge",
        ...preparePatch({ [path]: value }),
        revision: rev,
      });
  ...
  function distillToolEnabled(name: string): boolean {
    const tools = files["distill-config"]?.data?.tools as
  ...
  async function loadEnv() {
    try {
      const r = await mgmtRequest<{
        vars: Record<string, { set: boolean; source: string | null }>;
      }>("env-check", {});
...
<div class="pkg-forms">
  ...
  <details class="pkg-block" ontoggle={() => nsLoad("settings-global")}>
    ...
    <p class="hint">
      Saving here merges INSIDE the pi-plan namespace: editing planModel cannot erase
      btw/goal/plansDir (proven by unit test). Global scope.
    </p>
      ...
      <div class="frow">
        ...
        {#if f.type === "number"}
          <input
            class="num"
            type="number"
            value={String(nsValue("settings-global", "pi-plan", f.path) ?? "")}
            onchange={(e) => {
              const v = e.currentTarget.value;
              if (rejectBadNumber(f.label, e.currentTarget)) return;
              void savePlanField(f.path, v === "" ? undefined : Number(v));
            }}
          />
        {:else}
          <input
            class="grow mono"
            value={String(nsValue("settings-global", "pi-plan", f.path) ?? "")}
            onchange={(e) => void savePlanField(f.path, e.currentTarget.value)}
            placeholder="(absent)"
          />
        {/if}
  ...
  <details class="pkg-block" ontoggle={() => nsLoad("settings-global")}>
    ...
    <p class="hint">
      Models are plain strings — OpenRouter identifiers with / and :free are preserved exactly as
      typed. Changes apply on the next subagent spawn; no agents are launched by editing.
    </p>
    {#each roleNames("settings-global") as role (role)}
      <div class="frow">
        ...
        <textarea
          class="mono"
          rows={Math.max(2, roleModels("settings-global", role).length)}
        ...
        <button class="danger" onclick={() => void deleteRole("settings-global", role)}
          >Delete role</button
        >
      </div>
    {:else}
    ...
    <div class="frow">
      <input
        ...
        oninput={(e) => (newRoleName = e.currentTarget.value)}
      />
    ...
    <div class="frow">
      ...
      <textarea
        ...
        value={JSON.stringify(
          (files["settings-global"]?.data?.subagent as Record<string, unknown> | undefined)
            ?.agentModels ?? {},
          null,
          2,
        )}
        onchange={(e) => {
          try {
            ...
            void nsSave("settings-global", "subagent", { agentModels: v });
          } catch {
            statusNote.set("⚠ agentModels must be valid JSON");
            setTimeout(() => statusNote.set(""), 6000);
          }
        }}></textarea>
    </div>
    <p class="hint">
      Security options are pending: the installed security resolver reads an undocumented ctx
      property — file-backed toggles would be ineffective; disclosed, not faked.
    </p>
  </details>
  ...
  <details class="pkg-block" ontoggle={() => nsLoad("settings-global")}>
    ...
    <p class="hint">
      Per-tool pattern → allow/ask/deny maps. Rule order = insertion order shown; later matching
      rules override earlier ones. yolo (a launch flag) bypasses ask but never deny.
    </p>
      ...
      <div class="perm-tool">
        ...
        {#each permPatterns("settings-global", tool) as row, i (i)}
          <div class="frow">
            <input
              ...
              value={row.pattern}
              onchange={(e) => {
                const p = permPatterns("settings-global", tool);
                p[i] = { pattern: e.currentTarget.value, decision: p[i]?.decision ?? "ask" };
                void savePermissionTool("settings-global", tool, p);
              }}
            />
            <select
              value={row.decision}
              onchange={(e) => {
                const p = permPatterns("settings-global", tool);
                p[i] = { pattern: row.pattern, decision: e.currentTarget.value as Decision };
                void savePermissionTool("settings-global", tool, p);
              }}
            >
              <option value="allow">allow</option><option value="ask">ask</option><option
        ...
        <div class="frow">
          <button
            ...
            onclick={() => {
              const p = permPatterns("settings-global", tool);
              p.push({ pattern: "", decision: "ask" });
              void savePermissionTool("settings-global", tool, p);
            }}>Add rule</button
          >
          <button class="danger" onclick={() => void removePermissionTool("settings-global", tool)}
            >Remove tool</button
          >
        </div>
      </div>
    ...
    <div class="frow wrap">
      {#each lensBooleans as b (b)}
        <label class="check"
          ><input
            type="checkbox"
            checked={(files["lens-project"]?.data?.[b] as boolean) ?? false}
            onchange={(e) => void saveLensField(b, e.currentTarget.checked)}
          />
          {b}</label
        >
      {/each}
    ...
    <div class="frow wrap">
      {#each lensNumbers as n (n)}
        ...
        <input
          class="num"
          type="number"
          value={String(files["lens-project"]?.data?.[n] ?? "")}
          onchange={(e) => {
            const v = e.currentTarget.value;
            if (rejectBadNumber(n, e.currentTarget)) return;
            void saveLensField(n, v === "" ? undefined : Number(v));
          }}
        />
      {/each}
    </div>
    <p class="hint">
      includePatterns/excludePatterns and the separate piLensRenderer TUI switch: pending (coverage
      matrix). The renderer switch affects terminal Lens, not Leftleg.
    </p>
  </details>
  ...
  <details class="pkg-block" ontoggle={() => void load("tool-display-config").catch(() => {})}>
    ...
    <div class="frow">
      <label class="check"
        ><input
          type="checkbox"
          checked={(files["tool-display-config"]?.data?.enabled as boolean) ?? false}
          onchange={(e) => void saveToolDisplayField("enabled", e.currentTarget.checked)}
        /> enabled</label
      >
      <label class="check"
        ><input
          type="checkbox"
          checked={(files["tool-display-config"]?.data?.enableNativeUserMessageBox as boolean) ??
            false}
          onchange={(e) =>
            void saveToolDisplayField("enableNativeUserMessageBox", e.currentTarget.checked)}
        /> enableNativeUserMessageBox (TUI-only rendering)</label
      >
    </div>
    <p class="hint">
      registerToolOverrides / customToolOverrides and output-mode fields: pending (advanced). These
      are TUI rendering options — they do not change which tools the agent can call.
    </p>
  </details>
  ...
  <details class="pkg-block" ontoggle={() => void load("distill-config").catch(() => {})}>
    ...
    <div class="frow">
      <label class="check"
        ><input
          type="checkbox"
          checked={(files["distill-config"]?.data?.enabled as boolean) ?? false}
          onchange={(e) => void saveDistillField("enabled", e.currentTarget.checked)}
        /> enabled</label
      >
      <input
        class="mono"
        value={String(files["distill-config"]?.data?.model ?? "")}
        onchange={(e) => void saveDistillField("model", e.currentTarget.value || undefined)}
    ...
    <div class="frow wrap">
      {#each distillNumbers as n (n)}
        ...
        <input
          class="num"
          type="number"
          value={String(files["distill-config"]?.data?.[n] ?? "")}
          onchange={(e) => {
            const v = e.currentTarget.value;
            if (rejectBadNumber(n, e.currentTarget)) return;
            void saveDistillField(n, v === "" ? undefined : Number(v));
          }}
        />
      {/each}
      <label class="check"
        ><input
          type="checkbox"
          checked={(files["distill-config"]?.data?.summarizeErrors as boolean) ?? false}
          onchange={(e) => void saveDistillField("summarizeErrors", e.currentTarget.checked)}
        /> summarizeErrors</label
      >
    </div>
    ...
    <div class="frow wrap">
      <label class="check"
        ><input
          type="checkbox"
          checked={((files["distill-config"]?.data?.render as Record<string, unknown> | undefined)
            ?.enabled as boolean) ?? false}
          onchange={(e) => void saveDistillField("render.enabled", e.currentTarget.checked)}
        /> render.enabled</label
      >
      <label class="check"
        ><input
          type="checkbox"
          checked={((files["distill-config"]?.data?.render as Record<string, unknown> | undefined)
            ?.showPrompt as boolean) ?? false}
          onchange={(e) => void saveDistillField("render.showPrompt", e.currentTarget.checked)}
        /> showPrompt</label
      >
      <label class="check"
        ><input
          type="checkbox"
          checked={((files["distill-config"]?.data?.render as Record<string, unknown> | undefined)
            ?.showResult as boolean) ?? false}
          onchange={(e) => void saveDistillField("render.showResult", e.currentTarget.checked)}
        /> showResult</label
      >
    </div>
    ...
    {#each distillToolNames() as t (t)}
      <label class="check"
        ><input
          type="checkbox"
          checked={distillToolEnabled(t)}
          onchange={(e) => void saveDistillField(`tools.${t}.enabled`, e.currentTarget.checked)}
        />
        {t}</label
      >
    {:else}
  ...
  <details class="pkg-block">
    ...
    <p class="hint">
      Pi loads these as instruction sources (replace vs append semantics are pi's own). Edits apply
      on pi's reload rules — not injected into the running conversation. Raw edits preserve
      frontmatter.
    </p>
    {#each INSTRUCTION_TARGETS as t (t.target)}
      <div class="frow">
        <span class="flabel">{t.label}</span>
        <button class="ghost" onclick={() => void load(t.target, true).catch(() => {})}
          >{files[t.target] ? "Reload" : "Load"}</button
        >
      </div>
      {#if files[t.target]}
        <textarea
          ...
          oninput={(e) => {
            files = {
              ...
              [t.target]: {
                ...
                revision: files[t.target]?.revision ?? null,
                exists: true,
              },
            };
          }}></textarea>
  ...
  <details
    class="pkg-block"
    ontoggle={() => {
      void load("serena-global-yml", true).catch(() => {});
      void load("serena-project-yml", true).catch(() => {});
    }}
  >
    <summary>Serena (YAML — raw editors; comments and formatting are yours to preserve)</summary>
    <p class="hint">
      Language servers, ignored paths, read-only mode, tool inclusion/exclusion, modes, timeouts,
      symbol budgets live in these YAML files. Tab-indentation is refused. Serena worker restart
      applies changes.
    </p>
    ...
    <textarea
      class="mono yaml"
      rows={8}
      value={files["serena-global-yml"]?.raw ?? ""}
      onchange={(e) => {
        files = {
          ...files,
          ["serena-global-yml"]: {
            data: null,
            raw: e.currentTarget.value,
            revision: files["serena-global-yml"]?.revision ?? null,
            exists: true,
          },
        };
      }}></textarea>
    <div class="frow">
      <button
        onclick={() =>
          void mgmtRequest("write-raw", {
            target: "serena-global-yml",
            content: files["serena-global-yml"]?.raw ?? "",
            revision: revisionFor("serena-global-yml"),
          })
            .then(() => load("serena-global-yml", true))
            .then(flashSaved)
            .catch(noteSaveError)}>Save global YAML</button
      >
    </div>
    ...
    <textarea
      class="mono yaml"
      rows={8}
      value={files["serena-project-yml"]?.raw ?? ""}
      onchange={(e) => {
        files = {
          ...files,
          ["serena-project-yml"]: {
            data: null,
            raw: e.currentTarget.value,
            revision: files["serena-project-yml"]?.revision ?? null,
            exists: true,
          },
        };
      }}></textarea>
      ...
      <button
        onclick={() =>
          void mgmtRequest("write-raw", {
            ...
            content: files["serena-project-yml"]?.raw ?? "",
            revision: revisionFor("serena-project-yml"),
          })
            .then(() => load("serena-project-yml", true))
            .then(flashSaved)
            .catch(noteSaveError)}>Save project YAML</button
      >
    </div>
  </details>
    ...
    {#if envVars}
      <div class="frow wrap">
        {#each envVars as v (v.name)}
          <span class="chip" class:ok={v.set} class:dim={!v.set}
            >{v.name}: {v.set ? `set (${v.source})` : "not set"}</span
          >
        {/each}
      </div>
      <p class="hint">
        Lookup order: process env → cwd .env.local → cwd .env → agent .env.local → agent .env.
        Editing env files and OAuth/key replacement flows: pending (deliberate actions only; secrets
        are never read back into forms, logs, or the transcript).
      </p>
    {:else}
  ...
  <details class="pkg-block" ontoggle={() => nsLoad("settings-global")}>
      ...
      <div class="frow">
        ...
        {#if f.type === "number"}
          <input
            class="num"
            type="number"
            value={String(nsValue("settings-global", "pi-ref-tools", f.path) ?? "")}
            onchange={(e) => {
              const v = e.currentTarget.value;
              if (rejectBadNumber(f.label, e.currentTarget)) return;
              void nsSave("settings-global", "pi-ref-tools", {
                [f.path]: v === "" ? undefined : Number(v),
              });
            }}
          />
        {:else}
          <input
            ...
            onchange={(e) =>
              void nsSave("settings-global", "pi-ref-tools", {
                [f.path]: e.currentTarget.value || undefined,
              })}
            placeholder={f.hint}
          />
        {/if}
    ...
    {/each}
    <p class="hint">
      REF_API_KEY presence is shown under Web &amp; Ref; the private config file and CLI/env
      overrides take precedence over these values (shown as source info, pending).
    </p>
  </details>
</div>
