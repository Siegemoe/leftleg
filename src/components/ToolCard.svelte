<script lang="ts">
  ...
  async function openFile() {
    try {
      await openPathLocal(await resolvePath($projectDir, filePath));
    } catch (e) {
      transientNote(`Couldn't open file: ${e}`);
    }
  }
  ...
  let imageArgs = $derived.by(() => {
    ...
    try {
      return JSON.parse(item.args) as { aspect_ratio?: string };
    } catch {
      return null;
    }
  });
  ...
  async function openImage(path: string) {
    try {
      ...
      transientNote(`Couldn't open image: ${e}`);
    }
  }
  ...
  let liveElapsed = $derived(
    item.status === "running" && item.startedAt !== undefined
      ? formatDuration(Math.max(0, now - item.startedAt))
      : "",
  );
</script>
...
<div
  ...
  class:imggen={isImageGen}
>
  <button class="head" onclick={() => (expanded = !expanded)}>
    <span class="status-dot"></span>
    <span class="lbl"
      >{item.status === "running" ? `${label === item.name ? item.name : label}…` : label}</span
    >
    ...
    {#if diffStats}
      <span class="diffstat"
        ><span class="plus">+{diffStats.plus}</span>
        <span class="minus">−{diffStats.minus}</span></span
      >
    {/if}
        ...
        {#each imageSrcs as src, i (src)}
          {#if failedImages[imagePaths[i]]}
            <button
              class="imgchip mono"
              title={imagePaths[i]}
              onclick={() => void openImage(imagePaths[i])}
            >
          ...
          {:else}
            <button
              class="imgbtn"
              title="Open full size — {imagePaths[i]}"
              onclick={() => void openImage(imagePaths[i])}
            >
              <img
                {src}
    ...
    <div class="body">
      {#if item.args && item.args !== "{}"}
        <div class="kv">
          ...
          <pre class="mono args">{item.args}</pre>
        </div>
      {/if}
      {#if item.diff}
      ...
      {#if isImageGen && imagePaths.length > 0}
        <div class="actions">
          {#each imagePaths as p (p)}
            <button class="mono" onclick={() => void openImage(p)}
              >Open {p.split(/[\\/]/).pop()}</button
            >
          {/each}
        </div>
      {/if}
