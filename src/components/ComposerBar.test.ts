...
describe("composer bar", () => {
  it("leads the row with the active branch + dirty count", async () => {
    mocks.gitRepoInfo.mockResolvedValue({ repo: true, branch: "main", dirty: 3, toplevel: "/a" });
    projectDir.set("/a");
    instance = mount(ComposerBar, { target: document.body });
    flushSync();
    await vi.waitFor(() =>
    ...
    expect(document.body.firstElementChild?.classList.contains("git-wrap")).toBe(true);
  });
  ...
  it("orders branch chip, model pill, then think pill", async () => {
    ...
    expect(kids[0].classList.contains("git-wrap")).toBe(true);
    expect(kids[1].classList.contains("modelwrap")).toBe(true);
    expect((kids[2] as HTMLElement).tagName).toBe("BUTTON");
    expect(kids[2].textContent).toContain("think: medium");
  });
