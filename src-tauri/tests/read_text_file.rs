...
fn read_text_file_enforces_containment_tracked_and_binary_gates() {
    ...
    let ok = leftleg_lib::read_text_file_checked(&app, repo_root.to_str().unwrap(), "package.json")
        .unwrap();
    ...
    let err = leftleg_lib::read_text_file_checked(
        &app,
    ...
    let err = leftleg_lib::read_text_file_checked(&app, outside.to_str().unwrap(), "secret.md")
        .unwrap_err();
    ...
    let ok =
        leftleg_lib::read_text_file_checked(&app, jsrepo.to_str().unwrap(), "tool.js").unwrap();
    assert_eq!(ok.content, "console.log('hi');\n");
    assert_eq!(ok.loc, 1);
    let err =
        leftleg_lib::open_path_allowed(&app, jsrepo.join("tool.js").to_str().unwrap()).unwrap_err();
