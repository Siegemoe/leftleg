...
- `docs/` — `AGENT-PLAYBOOK.md` (step-by-step recipes for agent work), `ARCHITECTURE.md` (deep dive), `ROADMAP.md` (what's next)
...
2. **Svelte 5 syntax only:** `onclick`/`oninput` (not `on:click`), `$state`/`$derived`/`$effect` runes allowed. Inside `$derived.by`, always use `$storeName` (auto-subscription) — a bare store reference is the store _object_ and will crash with "not iterable" at minified runtime (this exact bug shipped once — see git history).
3. **Tauri 2 capabilities are explicit:** `src-tauri/capabilities/default.json` grants `core:default`, `dialog:default`, `opener:allow-open-url` + `opener:allow-default-urls`, `updater:default`, `process:default`, and `core:window:allow-*` for the custom title bar. Any new plugin needs a permission entry there or its invokes silently fail.
